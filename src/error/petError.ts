...
const petError = Object.freeze({
  auth: (err: VerifyErrors | Error, req: Request, res: Response, next: NextFunction) => {
  ...
  level: (err: Error, req: Request, res: Response, next: NextFunction) => {
    res.status(404).json(errTemplate.queryErr('에러발생 관리자에게 문의주세요'));
  },

  hungry: (err: Error, req: Request, res: Response, next: NextFunction) => {
